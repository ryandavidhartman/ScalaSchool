...
object ParallelCountChange {
  ...
  def moneyThreshold(startingMoney: Int): Threshold = (amount:Int, _) =>
    {
      amount <= (2*startingMoney) / 3
    }

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold = (_, coins: List[Int]) => {
    (3*coins.length) <= (2*totalCoins)
  }

  /** Threshold heuristic based on the starting money and the initial list of coins. */
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = (amount:Int, coins:List[Int]) => {
    (2*amount*coins.length) <= (startingMoney*allCoins.length)
  }
}
