...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = {
      val result = ReplaySubject[Try[List[String]]]()

      searchTerms subscribe (s => {
        val future = wikipediaSuggestion(s)
    ...
    val suggestionSubscription: Subscription = suggestions.observeOn(eventScheduler) subscribe {
      x =>
        {
          if (x.isSuccess)
            suggestionList.listData = x.get
          else
            status.text = "error" + x.get.toString
        }
    }

    val selections: Observable[String] = {

      val result = ReplaySubject[String]()
      button.clicks subscribe (observer => {
        if (suggestionList.selection.items.length == 1) {
          result.onNext(suggestionList.selection.items(0))
          result.onCompleted
        }
      })
      result
    ...
    val pages: Observable[Try[String]] = {
      val result = ReplaySubject[Try[String]]()
      selections subscribe (selection => {
        val future = wikipediaPage(selection)
        future onComplete {
          case Success(x) => result.onNext(Try(x))
          case Failure(e) => result.onError(e)
        }
      })
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x =>
        {
          if (x.isSuccess)
            editorpane.text = x.get
          else
            status.text = "eror" + x.get.toString
        }
    }
