...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
    ...
    def sanitized: Observable[String] = {
      ...
      obs subscribe (term => subject.onNext(term.replaceAll(" ", "_")))
      subject.onCompleted
      subject
    }
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = obs
      .map(t => Success(t))
    ...
    def timedOut(totalSec: Long): Observable[T] = {
      ...
      obs subscribe (o => result.onNext(o))
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      var result = Observable[Try[S]]()

      obs subscribe (observer => result = result ++ requestMethod(observer).recovered)
