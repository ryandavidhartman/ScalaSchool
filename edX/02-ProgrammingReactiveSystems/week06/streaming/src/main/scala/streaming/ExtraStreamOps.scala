...
trait ExtraStreamOps {
  ...
  implicit final class ExtraOps[T, M](val f: Flow[T, T, M]) {
    ...
    def logAllEvents (name: String): Flow[T, T, NotUsed] = {
      val l: GraphStage[FlowShape[T, T]] = new GraphStage[FlowShape[T, T]] {
        val in: Inlet[T] = Inlet[T](name = "logAllEvents.in")
        val out: Outlet[T] = Outlet[T](name = "logAllEvents.out")
        ...
        override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape)
          ...
          val paddedName: String = name.take(9).padTo(9, ' ').replace('\n', ' ')
          val level: Logging.LogLevel = Logging.InfoLevel
      ...
      val r: GraphStage[FlowShape[T, T]] = new GraphStage[FlowShape[T, T]] {
        val in: Inlet[T] = Inlet[T]("logAllEvents.in")
        val out: Outlet[T] = Outlet[T]("logAllEvents.out")
