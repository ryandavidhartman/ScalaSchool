...
object SimpleStreaming extends ExtraStreamOps {
  ...
  def mapToStrings(ints: Source[Int, NotUsed]): Source[String, NotUsed] = ints.map[String](_.toString)

  /** Filter elements which are even (use the modulo operator: `%`) */
  /*
  def filterEvenValues: Flow[Int, Int, NotUsed] = {
    val l: GraphStage[FlowShape[Int, Int]] = new GraphStage[FlowShape[Int, Int]] {
      val in: Inlet[Int] = Inlet[Int](name = "inlet")
      val out: Outlet[Int] = Outlet[Int](name = "outlet")
  ...
  def filterUsingPreviousFlowAndMapToStringsUsingTwoVias(
    ints: Source[Int, NotUsed], toString: Flow[Int, String, _]
  ): Source[String, NotUsed] = ints.via(filterEvenValues).via(toString)
   ...
   */
  def recoverSingleElement(ints: Source[Int, NotUsed]): Source[Int, NotUsed] = ints.recover {
