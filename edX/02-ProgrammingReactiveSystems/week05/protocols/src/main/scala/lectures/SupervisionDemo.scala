...
import akka.actor.typed.{ Behavior, ExtensibleBehavior, SupervisorStrategy, Signal }
import akka.actor.typed.scaladsl._

object SupervisionDemo {
    val actor: Behaviors.Receive[String] = Behaviors.receiveMessage[String] {
        case "okay" => Behaviors.same
        case "divide" => throw new ArithmeticException
        // anything else results in a MatchError
    }

    val supervisor: Behavior[Any] = Behaviors.setup[Any] { ctx =>
        ctx.spawnAnonymous(
    ...
    val supervisor2: Behavior[String] = Behaviors.setup[String] { ctx =>
    ...
    private class Restarter[T](initial: Behavior[T], behavior: Behavior[T]) extends ExtensibleBehavior[T] {
        def receive(ctx: akka.actor.typed.TypedActorContext[T], msg: T): Behavior[T] = {
            try {
        ...
        def receiveSignal(ctx: akka.actor.typed.TypedActorContext[T], msg: Signal): Behavior[T] =
