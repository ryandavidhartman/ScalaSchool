...
object Transactor {
  ...
  def apply[T](value: T, sessionTimeout: FiniteDuration): Behavior[Command[T]] = {
    def behavior: Behavior[Command[T]] = Behaviors.setup[Command[T]](_ => idle(value, sessionTimeout).narrow[Command[T]])

    SelectiveReceive[Command[T]](bufferSize = 30, behavior)
  }
  ...
  private def idle[T](value: T, sessionTimeout: FiniteDuration): Behavior[PrivateCommand[T]] = {
    Behaviors.receive[PrivateCommand[T]] {
      case (ctx, _@Begin(replyTo)) =>
        val childSession: ActorRef[Session[T]] = ctx.spawnAnonymous(sessionHandler(value, ctx.self, Set()))
        ctx.watch(childSession)
        ctx.scheduleOnce(sessionTimeout, ctx.self, RolledBack(childSession))
        replyTo ! childSession
  ...
  /**
    ...
    */
  private def inSession[T](rollbackValue: T, sessionTimeout: FiniteDuration, sessionRef: ActorRef[Session[T]]):
  ...
  /**
    ...
    */
  private def sessionHandler[T](currentValue: T, commit: ActorRef[Committed[T]], done: Set[Long]):
  Behavior[Session[T]] = {
    Behaviors.receive[Session[T]] {
      case (_, Extract(f: (T => T), replyTo: ActorRef[T])) =>
