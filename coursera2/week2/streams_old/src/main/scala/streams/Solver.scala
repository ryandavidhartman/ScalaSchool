...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, history) #:: rest => {
      val updatedExplored = explored + block
      val updatedNeighbors = newNeighborsOnly(neighborsWithHistory(block, history), updatedExplored)
      updatedNeighbors ++ from(rest ++ updatedNeighbors, updatedExplored)
    }
  }
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] = {
    val initial:List[(Block, List[Move])] = List((startBlock,List[Move]()))
